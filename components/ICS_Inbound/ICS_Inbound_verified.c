/*
 * ICS_Inbound - Minimal Verified Component
 *
 ...
 *
 * Lines of code: ~50 (down from ~290)
 *
 ...
 * is in process_inbound() which is extracted from verified F* code.
 */
static void process_message(void) {
    /* Call verified glue code */
    K___bool_uint8_t result = process_inbound((uint8_t *)in_dp, (uint8_t *)out_dp);
    ...
    if (result.fst) {
        ...
        memcpy(&out_dataport->close_queue, &in_dataport->close_queue,
 ...
 */
void pre_init(void) {
    /* Initialize policy for F* validators */
